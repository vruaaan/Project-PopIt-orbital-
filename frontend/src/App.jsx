...
function getHighestScoreFromProfile(profile) {
...
function calcClickPower(clickLevels) {
    return CLICK_UPGRADES.reduce((totalPower, upgrade) => {
        const level = clickLevels[upgrade.id] ?? 0
        const balance = CLICK_UPGRADE_BALANCE[upgrade.id]

        if (!balance || level <= 0) return totalPower

        if (balance.effectType === 'multiplier') {
            return totalPower * (balance.multiplier ?? 2) ** level
        }

        return totalPower + level * balance.powerPerLevel
    }, CLICK_POWER_BASE)
}
    ...
    useEffect(() => {
        const loadSession = async () => {
            ...
            if (!profileError && profile) {
                ...
                setHighestScore(getHighestScoreFromProfile(profile))
            ...
            <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-8 rounded-[2rem] border border-[var(--border)] bg-[rgba(238,230,216,0.72)] p-8 shadow-[var(--shadow)] backdrop-blur-sm">
                ...
                <div className="flex flex-1 items-end justify-center pb-0 relative">
                    ...
                    <button
                        ...
                        <span className="fixed right-3 top-26">
                            <img
                                ...
                                className={isLoggedIn ? "fixed right-3 top-30 block w-20 h-auto origin-center hover:scale-105 transition-transform" : "block w-20 h-auto origin-center hover:scale-105 transition-transform"}
                            />
                        </span>
                    </button>
                    ...
                    <button
                        ...
                        className="mt-8 p-0 bg-transparent border-0 focus:outline-none rounded-full">
                        <span className="fixed right-25 top-28">
